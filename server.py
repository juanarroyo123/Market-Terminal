...
def fetch_calendar():
    ...
    COUNTRIES=("USD","EUR","GBP","CAD","CNY","JPY")
    # Country name → currency code (para Trading Economics)
    CNAME_MAP={"United States":"USD","Euro Area":"EUR","European Union":"EUR",
               "United Kingdom":"GBP","Canada":"CAD","China":"CNY","Japan":"JPY"}

    # Fuente 1: Trading Economics — demo público, funciona desde IPs cloud
    try:
        ...
        if r.status_code==200 and r.text.startswith("["):
            ...
            te_added=0
            for item in raw:
                country=CNAME_MAP.get(item.get("Country",""),"")
                if not country: continue
                imp=item.get("Importance",0)
                if imp<2: continue  # 3=High, 2=Medium, 1=Low
                imp_str="High" if imp>=3 else "Medium"
                date_raw=item.get("Date","")
                if not date_raw: continue
                data.append({"country":country,"impact":imp_str,
                    "date":date_raw,
                ...
                te_added+=1
            print(f"TradingEconomics OK: {te_added} eventos")
    except Exception as e:
        print(f"TradingEconomics error: {e}")

    # Fuente 2: ForexFactory JSON (cuando no bloquea)
    for cal_url in ["https://nfs.faireconomy.media/ff_calendar_thisweek.json",
                    "https://nfs.faireconomy.media/ff_calendar_nextweek.json"]:
        try:
            r=requests.get(cal_url,timeout=10,headers=HEADERS)
            if r.status_code==200 and len(r.text)>100:
                raw=r.json()
                data.extend(raw)
                print(f"FF OK: {cal_url[-20:]} — {len(raw)} eventos")
        except Exception as e:
            print(f"FF error: {e}")

    # Fuente 3: Nasdaq Economic Calendar (solo USD, alta importancia)
    try:
        nasdaq_added=0
        for offset in range(8):
            ...
            try:
                r=requests.get(url,timeout=8,headers={**HEADERS,"Origin":"https://www.nasdaq.com"})
                if r.status_code==200:
                    rows=r.json().get("data",{}).get("rows",[]) or []
                    for row in rows:
                        imp=row.get("importance","")
                        if imp not in ("High","***"): continue
                        data.append({"country":"USD","impact":"High",
                            "date":d+"T"+(row.get("time") or "12:00")+":00",
        ...
        if nasdaq_added: print(f"Nasdaq OK: {nasdaq_added} eventos")
    except Exception as e:
        print(f"Nasdaq error: {e}")

    if not data:
        print("Calendario: todas las fuentes fallaron — manteniendo caché")

    try:
        imp_map={"High":"Alto","Medium":"Medio","Low":"Bajo",
                 "Holiday":"Bajo","Non-Economic":"Bajo"}
        for item in data:
            # Soporte para formato Nasdaq (sin country field, solo USD)
            country=item.get("country","USD")
            if country not in COUNTRIES: continue
            imp_raw=item.get("impact","Low")
            if imp_raw not in ("High","Medium"): continue  # High+Medium = equivalente a High en Investing.com
    ...
    try:
        ...
        for fn, key in [
            ...
            try:
                result = fn()
                with LOCK:
                    # No sobreescribir calendario con vacío
                    if key == "calendar" and not result:
                        pass
                    else:
                        CACHE[key] = result
                gc.collect()
...
def update_slow():
    ...
    try:
        ...
        with LOCK:
            # Nunca sobreescribir calendario con lista vacía — mantener datos buenos
            cal_new = results.pop("calendar", None)
            if cal_new:
                CACHE["calendar"] = cal_new
            CACHE.update(results)
