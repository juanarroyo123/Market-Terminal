...
def fetch_calendar():
    ...
    # Fuente 1: ForexFactory JSON (a veces funciona)
    ...
    # Fuente 2: Nasdaq Economic Calendar (API pública, no bloqueada)
    if not data:
        try:
            ...
            for offset in range(7):
                ...
                r=requests.get(url,timeout=10,headers={**HEADERS,"Origin":"https://www.nasdaq.com"})
                if r.status_code==200:
                    ...
                    for row in rows:
                        ...
                        data.append({"country":"USD","impact":"High",
                            "date":d+"T"+row.get("time","12:00")+":00",
                            "title":row.get("eventName",""),
                            "actual":row.get("actual",""),
                            "forecast":row.get("consensus",""),
                            "previous":row.get("previous",""),
                            "_nasdaq":True})
            print(f"Nasdaq OK: {len(data)} eventos")
        except Exception as e:
            print(f"Nasdaq error: {e}")

    # Fuente 3: FMP Economic Calendar (fallback final)
    if not data:
        try:
            from_d=today.strftime("%Y-%m-%d")
            to_d=(today+timedelta(days=7)).strftime("%Y-%m-%d")
            url=f"https://financialmodelingprep.com/api/v3/economic_calendar?from={from_d}&to={to_d}&apikey=demo"
            r=requests.get(url,timeout=10,headers=HEADERS)
            if r.status_code==200:
                for item in r.json():
                    c_map={"US":"USD","EU":"EUR","GB":"GBP","CA":"CAD","CN":"CNY","JP":"JPY"}
                    c=c_map.get(item.get("country",""),"")
                    if not c: continue
                    data.append({"country":c,"impact":"High" if item.get("impact")=="High" else "Medium",
                        "date":item.get("date",""),"title":item.get("event",""),
                        "actual":item.get("actual",""),"forecast":item.get("estimate",""),
                        "previous":item.get("previous","")})
            print(f"FMP OK: {len(data)} eventos")
        except Exception as e:
            print(f"FMP error: {e}")
