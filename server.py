...
TOP_SP500={
    ...
    "BofA":"BAC","Costco":"COST","Thermo Fisher":"TMO","Oracle":"ORCL",
    "McDonald's":"MCD","Salesforce":"CRM","Goldman":"GS","Morgan Stanley":"MS",
    "Accenture":"ACN","Adobe":"ADBE","Cisco":"CSCO","IBM":"IBM",
    "GE":"GE","Caterpillar":"CAT","Honeywell":"HON","Boeing":"BA",
    "BlackRock":"BLK","Qualcomm":"QCOM","TI":"TXN","ServiceNow":"NOW",
    "ASML":"AMAT","Micron":"MU","Palantir":"PLTR","Intuit":"INTU",
    "Lowe's":"LOW","Amgen":"AMGN","Pepsico":"PEP","KO":"KO",
}
...
def _load_prev_closes():
    ...
    global _PREV_CLOSES
    try:
        syms=list(TICKERS.values())+list(TOP_SP500.values())
...
def fetch_market_breadth():
    sp500_sample = [
        ...
        'CRM','BAC','NFLX','AMD','WMT','ACN','TMO','LIN','MCD','GE',
        'ORCL','NOW','TXN','PM','DHR','NEE','HON','IBM','AMGN','CAT',
        'QCOM','LOW','UPS','SPGI','BLK','AMAT','DE','VRTX','REGN','ADI',
    ]
    try:
        ...
        for sym in sp500_sample:
            try:
                tk = yf.Ticker(sym)
                s_df = tk.history(period='1y', interval='1d', auto_adjust=True)
...
def fetch_calendar():
    ...
    try:
        ...
        for item in data:
            ...
            if imp_raw not in ("High","Medium"): continue
...
def fetch_earnings():
    ...
    TOP_CAPS = [
        ...
        # Añadir más que suelen reportar
        "WFC","C","USB","TFC","PNC","RF","FITB","KEY","CFG","HBAN",
        "STT","BK","SCHW","CME","ICE","CBOE","NDAQ",
        "MMC","AON","AJG","MET","PRU","AFL","ALL","PGR","TRV",
        "RTX","NOC","GD","HII","TDG","HWM","SPR",
        "EMR","ETN","PH","ROK","AME","IR","XYL",
        "APD","LIN","PPG","SHW","ECL","DD","DOW","LYB",
...
def fetch_news():
    ...
    for source,url in NEWS_SOURCES:
        try:
            ...
            for entry in feed.entries[:30]:  # Mas entradas por feed
    ...
    titulos_en = [a["title_en"] for a in articles[:60] if a["title_en"]]
    if titulos_en:
        try:
            traducciones = traducir_batch(titulos_en)
            for a in articles[:60]:
                a["title"] = traducciones.get(a["title_en"], a["title_en"])
        except:
            pass
    return articles
...
def fetch_social():
    ...
    all_posts = all_posts[:120]

    # Traducir EN->ES igual que fetch_news — batch en paralelo
    textos_en = [p["text_en"] for p in all_posts[:80] if p.get("text_en")]
    if textos_en:
        try:
            traducciones = traducir_batch(textos_en)
            for p in all_posts[:80]:
                traducido = traducciones.get(p.get("text_en", ""), "")
                if traducido and len(traducido) > 10:
                    p["text"] = traducido
        except:
            pass  # si falla traduccion se queda en ingles
...
def update_all():
    ...
    try:
        ...
        threads=[threading.Thread(target=fn,daemon=True) for fn in [_m,_f,_p,_c,_e,_o,_n,_s,_t,_br,_se]]
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] OK — "
              f"{len(results.get('news',[]))} noticias | "
              f"{len(results.get('calendar',[]))} eventos | "
              f"{len(results.get('options',[]))} opciones")
...
if __name__=="__main__":
    ...
    _load_prev_closes()  # Cargar cierres anteriores antes del primer ciclo
    update_all()
